...
do
	...
	wget -O ormxml/orm_$cnt_node.osm "http://overpass-api.de/api/map?data=[out:xml];(area['ISO3166-1:alpha2'=$cnt][admin_level=2];)->.a;(node[railway](area.a););(._;>;);out;"
	wget -O ormxml/orm_$cnt_way.osm "http://overpass-api.de/api/map?data=[out:xml];(area['ISO3166-1:alpha2'=$cnt][admin_level=2];)->.a;(way[railway](area.a););(._;>;);out;"
	wget -O ormxml/orm_$cnt_relation.osm "http://overpass-api.de/api/map?data=[out:xml];(area['ISO3166-1:alpha2'=$cnt][admin_level=2];)->.a;(relation[railway](area.a););(._;>;);out;"
done
